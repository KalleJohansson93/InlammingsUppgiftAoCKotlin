...
fun decoding(input: List<String>) : Int { // Den här blev inte så snygg men fungerar
    val sakerAttDecoda: List<String> = input.map { it.substring(it.indexOf('|') + 2, it.length) }
    var räknare = 0

    for (it in sakerAttDecoda) {

        val a: Int = it.indexOfFirst { it == ' ' }
...
fun förbättradDecoding(input: List<String>) : Int {
    val antalAvTalen1478 = input.sumOf {
        it.substringAfter(" | ")
            .split(' ')
            .count { it.length == 2 || it.length == 4 || it.length == 3 || it.length == 7 }
    }
    return antalAvTalen1478
}
